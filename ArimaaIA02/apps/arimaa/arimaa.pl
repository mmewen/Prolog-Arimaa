...
% default call
%get_moves([[[1,3],[2,3]],[[0,3],[1,3]],[[0,4],[0,3]],[[0,3],[0,4]]], Gamestate, Board).
...
apply_trap_3([[Type,Color]|Gamestate],Board2,Gamestate,[[Row,Col,Type,Color]|_], Board) :- is_on_trap_without_friend([Row,Col], Board), replace_3(Board2, [Row,Col,Type,Color], Board).
...
get_taken_piece_score(S,[[Type,silver] | List]) :- get_taken_piece_score(S1,List), strength(S2,Type), S is S1+S2.
get_taken_piece_score(S,[[_,gold] | List]) :- get_taken_piece_score(S,List).
...
get_score(S,Gamestate,Board) :- get_score2(S1,Board), get_taken_piece_score(S2,Gamestate), S is S1+S2*20.
get_score2(0,[]).
